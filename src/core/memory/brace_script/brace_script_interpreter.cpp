...
public:
    BraceScriptCoroutine()
        : CoroutineWithBoostContext::Coroutine(4 * 1024 * 1024), OnRoutine(nullptr),
          m_RuntimeStack(), m_MsgId() {}
...
private:
        ...
        if (pCurrent == &m_ScriptCoroutine) {
            for (auto&& coro : m_MessageHandlerCoroutines) {
                ...
                if (it != m_MessageQueues.end() &&
                    (it->second.size() > 0 || !coro->IsTerminated())) {
                    coro->TryStart();
                }
            }
        }
    ...
    void HandleMessage(const std::string& msgId,
                       const std::shared_ptr<Brace::FunctionExecutor>& exer) {
        ...
        if (it != m_MessageQueues.end() && it->second.size() > 0) {
            ...
            for (auto&& arg : msgArgs) {
                ...
                if (const bool* pBoolVal = std::get_if<bool>(&arg)) {
                    Brace::VarSetBoolean(*m_pBraceScript->GlobalVariables(), pArgInfo->Type,
                                         pArgInfo->VarIndex, *pBoolVal);
                } else if (const int64_t* pI64Val = std::get_if<int64_t>(&arg)) {
                    Brace::VarSetI64(*m_pBraceScript->GlobalVariables(), pArgInfo->Type,
                                     pArgInfo->VarIndex, *pI64Val);
                } else if (const uint64_t* pU64Val = std::get_if<uint64_t>(&arg)) {
                    Brace::VarSetU64(*m_pBraceScript->GlobalVariables(), pArgInfo->Type,
                                     pArgInfo->VarIndex, *pU64Val);
                } else if (const double* pDoubleVal = std::get_if<double>(&arg)) {
                    Brace::VarSetF64(*m_pBraceScript->GlobalVariables(), pArgInfo->Type,
                                     pArgInfo->VarIndex, *pDoubleVal);
                } else if (const std::string* pStrVal = std::get_if<std::string>(&arg)) {
                    Brace::VarSetStr(*m_pBraceScript->GlobalVariables(), pArgInfo->Type,
                                     pArgInfo->VarIndex, *pStrVal);
                } else if (pArgInfo->Type == Brace::BRACE_DATA_TYPE_OBJECT &&
                           std::holds_alternative<std::shared_ptr<void>>(arg)) {
                    auto&& objVal = std::get<std::shared_ptr<void>>(arg);
                    Brace::VarSetObject(*m_pBraceScript->GlobalVariables(), pArgInfo->VarIndex,
                                        objVal);
                } else {
                    // unexpected
                }
    ...
    bool RunCallbackImpl(std::string&& msg, MessageArgs&& args) {
        ...
        if (it != m_CallbackExers.end()) {
            ...
            for (auto&& arg : args) {
                ...
                auto* pArgInfo = exer->ArgInfo(argIx);
                if (const bool* pBoolVal = std::get_if<bool>(&arg)) {
                    Brace::VarSetBoolean(*m_pCallbackBraceScript->GlobalVariables(), pArgInfo->Type,
                                         pArgInfo->VarIndex, *pBoolVal);
                } else if (const int64_t* pI64Val = std::get_if<int64_t>(&arg)) {
                    Brace::VarSetI64(*m_pCallbackBraceScript->GlobalVariables(), pArgInfo->Type,
                                     pArgInfo->VarIndex, *pI64Val);
                } else if (const uint64_t* pU64Val = std::get_if<uint64_t>(&arg)) {
                    Brace::VarSetU64(*m_pCallbackBraceScript->GlobalVariables(), pArgInfo->Type,
                                     pArgInfo->VarIndex, *pU64Val);
                } else if (const double* pDoubleVal = std::get_if<double>(&arg)) {
                    Brace::VarSetF64(*m_pCallbackBraceScript->GlobalVariables(), pArgInfo->Type,
                                     pArgInfo->VarIndex, *pDoubleVal);
                } else if (const std::string* pStrVal = std::get_if<std::string>(&arg)) {
                    Brace::VarSetStr(*m_pCallbackBraceScript->GlobalVariables(), pArgInfo->Type,
                                     pArgInfo->VarIndex, *pStrVal);
                } else if (pArgInfo->Type == Brace::BRACE_DATA_TYPE_OBJECT &&
                           std::holds_alternative<std::shared_ptr<void>>(arg)) {
                    auto&& objVal = std::get<std::shared_ptr<void>>(arg);
                    Brace::VarSetObject(*m_pCallbackBraceScript->GlobalVariables(),
                                        pArgInfo->VarIndex, objVal);
                } else {
                    // unexpected
                }
                ++argIx;
            ...
            exer->Run(*m_pCallbackBraceScript->GlobalVariables(),
                      *m_pCallbackBraceScript->GlobalVariables());
            ret = true;
        }
        return ret;
    }
...
private:
    BraceScriptManager()
        : m_ScriptTxt(), m_ScriptQueue(), m_MessageQueues(), m_CommandQueue(), m_Imports(),
          m_IsQuitting(false), m_pBuffer(nullptr), m_DslFiles(), m_pBraceScript(nullptr),
          m_ScriptCoroutine(), m_MessageHandlerCoroutines(), m_CoroutineExers(),
...
protected:
    virtual bool LoadFunction(const Brace::FuncInfo& outerFunc,
                              const DslData::FunctionData& funcData,
                              Brace::OperandLoadtimeInfo& resultInfo,
                              Brace::BraceApiExecutor& executor) override {
        if (funcData.IsHighOrder()) {
            ...
            // error
            std::stringstream ss;
            ss << "expected oncallback(msg){...};" << funcData.GetId() << " line "
               << funcData.GetLine();
            LogError(ss.str());
            return false;
        }
    }
    virtual bool LoadStatement(const Brace::FuncInfo& outerFunc,
                               const DslData::StatementData& funcData,
                               Brace::OperandLoadtimeInfo& resultInfo,
                               Brace::BraceApiExecutor& executor) override {
        bool hasError = false;
        if (funcData.GetFunctionNum() == 2) {
            ...
            if (f1 && !f1->IsHighOrder() && f1->HaveParam() && f2 && f2->IsHighOrder() &&
                f2->HaveStatement()) {
                ...
                for (int ix = 0; ix < callData.GetParamNum(); ++ix) {
                    ...
                    if (p->GetSyntaxType() == DslData::ISyntaxComponent::TYPE_FUNCTION) {
                        ...
                        if (pf->IsOperatorParamClass() && pf->GetId() == ":") {
                            ...
                            if (pti.IsRef) {
                                int varIndex =
                                    AllocVariable(name, Brace::BRACE_DATA_TYPE_REF,
                                                  Brace::PREDEFINED_BRACE_OBJECT_TYPE_NOTOBJ);
                                newFunc->VarInitInfo.ReferenceVars[varIndex] = Brace::ReferenceInfo(
                                    pti.Type, pti.ObjectTypeId, INVALID_INDEX, nullptr);
                                newFunc->Params.push_back(Brace::ParamRetInfo(
                                    name, pti.Type, pti.ObjectTypeId, varIndex, true));
                            } else {
                                int varIndex = AllocVariable(name, pti.Type, pti.ObjectTypeId);
                                newFunc->Params.push_back(Brace::ParamRetInfo(
                                    name, pti.Type, pti.ObjectTypeId, varIndex, false));
                            }
                        }
                    } else {
                        hasError = true;
...
protected:
    virtual bool LoadFunction(const Brace::FuncInfo& outerFunc,
        ...
        if (funcData.IsHighOrder()) {
            ...
            // error
            std::stringstream ss;
            ss << "expected onmessage(msg[, pool_num]){...};" << funcData.GetId() << " line "
               << funcData.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    WaitExp(Brace::BraceScript& interpreter, bool forCallback)
        : Brace::SimpleBraceApiBase(interpreter), m_ForCallback(forCallback) {}
        ...
        for (auto&& argInfo : argInfos) {
            if (argInfo.Type == Brace::BRACE_DATA_TYPE_INT32) {
                ...
                if (v <= 60000) {
                    auto ccv = lcv;
                    while (
                        std::chrono::duration_cast<std::chrono::milliseconds>(ccv - lcv).count() <
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (v.length() > 2 && v[0] == '0' && v[1] == 'x')
            Brace::VarSetUInt64((resultInfo.IsGlobal ? gvars : lvars), resultInfo.VarIndex,
                                std::stoull(v.substr(2, v.length() - 2), nullptr, 16));
        else
            Brace::VarSetUInt64((resultInfo.IsGlobal ? gvars : lvars), resultInfo.VarIndex,
                                std::stoull(v, nullptr, 16));
    }
};
...
protected:
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (argInfos.size() == 2) {
            ...
            int precise = static_cast<int>(Brace::VarGetI64((argInfo2.IsGlobal ? gvars : lvars),
...
protected:
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        const std::string& v =
            Brace::VarGetString((argInfo.IsGlobal ? gvars : lvars), argInfo.VarIndex);
        Brace::VarSetDouble((resultInfo.IsGlobal ? gvars : lvars), resultInfo.VarIndex,
                            std::stod(v));
    }
};
...
protected:
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        std::stringstream ss;
        for (auto&& argInfo : argInfos) {
            std::string v = Brace::VarGetStr((argInfo.IsGlobal ? gvars : lvars), argInfo.Type,
                                             argInfo.VarIndex);
            ss << v;
        }
        Brace::VarSetString((resultInfo.IsGlobal ? gvars : lvars), resultInfo.VarIndex, ss.str());
...
protected:
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        const std::string& str =
            Brace::VarGetString((strInfo.IsGlobal ? gvars : lvars), strInfo.VarIndex);
        bool ret = false;
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        std::stringstream ss;
        ss << "expected strindexof(string, string, int) or strindexof(string, string) ! line: "
           << data.GetLine();
        LogError(ss.str());
        return false;
    }
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (argInfos.size() == 3) {
            ...
            pos = static_cast<size_t>(Brace::VarGetU64((posInfo.IsGlobal ? gvars : lvars),
                                                       posInfo.Type, posInfo.VarIndex));
        }
        const std::string& str =
            Brace::VarGetString((strInfo.IsGlobal ? gvars : lvars), strInfo.VarIndex);
        const std::string& str2 =
            Brace::VarGetString((strInfo2.IsGlobal ? gvars : lvars), strInfo2.VarIndex);
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        std::stringstream ss;
        ss << "expected strlastindexof(string, string, int) or strlastindexof(string, string) ! "
              "line: "
           << data.GetLine();
        LogError(ss.str());
        return false;
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        std::stringstream ss;
        ss << "expected substr(string, pos, count) or substr(string, pos) or substr(string) ! "
              "line: "
           << data.GetLine();
        LogError(ss.str());
        return false;
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (argInfos.size() == 3) {
            ...
            count = static_cast<size_t>(Brace::VarGetU64((countInfo.IsGlobal ? gvars : lvars),
                                                         countInfo.Type, countInfo.VarIndex));
        }
        const std::string& str =
            Brace::VarGetString((strInfo.IsGlobal ? gvars : lvars), strInfo.VarIndex);
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 3) {
            ...
            if (Brace::IsStringType(argInfo.Type) && Brace::IsStringType(argInfo2.Type) &&
                Brace::IsStringType(argInfo3.Type)) {
                resultInfo.Type = Brace::BRACE_DATA_TYPE_STRING;
                resultInfo.ObjectTypeId = Brace::PREDEFINED_BRACE_OBJECT_TYPE_NOTOBJ;
                resultInfo.Name = GenTempVarName();
                resultInfo.VarIndex =
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        const std::string& str3 =
            Brace::VarGetString((strInfo3.IsGlobal ? gvars : lvars), strInfo3.VarIndex);
        std::string r = str;
...
protected:
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        std::swap(*pArr, arr);
        Brace::VarSetObject((resultInfo.IsGlobal ? gvars : lvars), resultInfo.VarIndex,
                            std::shared_ptr<void>(pArr));
    }
};
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (Brace::IsObjectType(argInfo.Type) &&
                argInfo.ObjectTypeId == CUSTOM_BRACE_OBJECT_TYPE_STR_ARRAY &&
                Brace::IsStringType(argInfo2.Type)) {
                resultInfo.Type = Brace::BRACE_DATA_TYPE_STRING;
                resultInfo.ObjectTypeId = Brace::PREDEFINED_BRACE_OBJECT_TYPE_NOTOBJ;
                resultInfo.Name = GenTempVarName();
                resultInfo.VarIndex =
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 1) {
            ...
            if (objTypeId >= CUSTOM_BRACE_OBJECT_TYPE_STR_ARRAY &&
                objTypeId <= CUSTOM_BRACE_OBJECT_TYPE_BOOL_ARRAY) {
                isArray = true;
            }
            if (!Brace::IsStringType(ai1.Type) || !isArray) {
                std::stringstream ss;
                ss << "expected loadfiletoarray(file_path, typetag(array_type)) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
            }
...
private:
    ...
    inline void DoLoad(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                       const Brace::OperandRuntimeInfo& resultInfo,
                       const std::vector<std::string>& result) const {
        using ArrayType = ArrayT<TypeT>;
        auto* arrayObj = new ArrayType();
        Brace::VarSetObject((resultInfo.IsGlobal ? gvars : lvars), resultInfo.VarIndex,
                            std::shared_ptr<void>(arrayObj));
        auto& arr = *arrayObj;
    ...
    template <typename TypeT>
    inline void DoLoadEmpty(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                            const Brace::OperandRuntimeInfo& resultInfo) const {
        using ArrayType = ArrayT<TypeT>;
        auto* arrayObj = new ArrayType();
        Brace::VarSetObject((resultInfo.IsGlobal ? gvars : lvars), resultInfo.VarIndex,
                            std::shared_ptr<void>(arrayObj));
...
private:
    template <typename TypeT>
    inline void ArrayToStrArray(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                                const std::shared_ptr<void>& arrayWrap,
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 1) {
            ...
            if (!Brace::IsStringType(ai1.Type) || !isHash) {
                std::stringstream ss;
                ss << "expected loadhashtable(file_path, typetag(hash_type)) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
            }
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!isHash || !Brace::IsStringType(ai2.Type)) {
                std::stringstream ss;
                ss << "expected savehashtable(int_xxx_hash, file_path) or "
                      "savememinfo(str_xxx_hash, file_path) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
            }
...
private:
    template <typename KeyTypeT, typename ValueTypeT>
    inline void HashtableToFStrArray(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                                     const std::shared_ptr<void>& hashWrap,
                                     std::vector<std::string>& list) const {
        using HashType = HashtableT<KeyTypeT, ValueTypeT>;
...
public:
    CalcNewItemsExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter), m_ObjectCategory(INVALID_ID) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isHash) {
                std::stringstream ss;
                ss << "expected calcnewitems(int_xxx_hash, int_xxx_hash) or "
                      "calcnewitems(str_xxx_hash, str_xxx_hash) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
            }
...
private:
    ...
    inline void DoCalc(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                       const std::shared_ptr<void>& hashWrap0,
                       const std::shared_ptr<void>& hashWrap1,
                       const Brace::OperandRuntimeInfo& resultInfo) const {
...
public:
    CalcSameItemsExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter), m_ObjectCategory(INVALID_ID) {}
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isHash) {
                std::stringstream ss;
                ss << "expected calcsameitems(int_xxx_hash, int_xxx_hash) or "
                      "calcnewitems(str_xxx_hash, str_xxx_hash) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
...
public:
    CalcItemsUnionExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter), m_ObjectCategory(INVALID_ID) {}
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isHash) {
                std::stringstream ss;
                ss << "expected calcitemsunion(int_xxx_hash, int_xxx_hash) or "
                      "calcsumitems(str_xxx_hash, str_xxx_hash) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
            }
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isHash) {
                std::stringstream ss;
                ss << "expected itemsadd(int_xxx_hash, int_xxx_hash) or itemsadd(str_xxx_hash, "
                      "str_xxx_hash) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
...
private:
    ...
    template <typename HashObjT>
    inline void DoBoolCalc(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                           const std::shared_ptr<void>& hashWrap0,
                           const std::shared_ptr<void>& hashWrap1,
                           const Brace::OperandRuntimeInfo& resultInfo) const {
        auto* hashObj = new HashObjT();
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isHash) {
                std::stringstream ss;
                ss << "expected itemssub(int_xxx_hash, int_xxx_hash) or itemssub(str_xxx_hash, "
                      "str_xxx_hash) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
        ...
        if (nullptr != hashObj0 && nullptr != hashObj1) {
            ...
            for (auto&& pair : hash1) {
                ...
                if (it != hash0.end()) {
                    if (hash.find(pair.first) == hash.end()) {
                        hash.insert(
                            std::make_pair(pair.first, pair.second == it->second ? false : true));
                    }
                }
            }
        }
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isHash) {
                std::stringstream ss;
                ss << "expected itemsmul(int_xxx_hash, int_xxx_hash) or itemsmul(str_xxx_hash, "
                      "str_xxx_hash) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isHash) {
                std::stringstream ss;
                ss << "expected itemsdiv(int_xxx_hash, int_xxx_hash) or itemsdiv(str_xxx_hash, "
                      "str_xxx_hash) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isArray) {
                std::stringstream ss;
                ss << "expected arrayadd(int_array, int_array) or arrayadd(float_array, "
                      "float_array) or arrayadd(bool_array, bool_array) or arrayadd(str_array, "
                      "str_array) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
...
private:
    ...
    inline void DoCalc(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                       const std::shared_ptr<void>& arrayWrap0,
                       const std::shared_ptr<void>& arrayWrap1,
                       const Brace::OperandRuntimeInfo& resultInfo) const {
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isArray) {
                std::stringstream ss;
                ss << "expected arraysub(int_array, int_array) or arraysub(float_array, "
                      "float_array) or arraysub(bool_array, bool_array) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
            }
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isArray) {
                std::stringstream ss;
                ss << "expected arraymul(int_array, int_array) or arraymul(float_array, "
                      "float_array) or arraymul(bool_array, bool_array) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
            }
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() == 2) {
            ...
            if (!Brace::IsObjectType(ai1.Type) || !Brace::IsObjectType(ai2.Type) ||
                ai1.ObjectTypeId != ai2.ObjectTypeId || !isArray) {
                std::stringstream ss;
                ss << "expected arraydiv(int_array, int_array) or arraydiv(float_array, "
                      "float_array) ! line: "
                   << data.GetLine();
                LogError(ss.str());
                return false;
...
public:
    ArrayModifyExp(Brace::BraceScript& interpreter)
        : Brace::AbstractBraceApi(interpreter), m_IteratorIndex(INVALID_INDEX), m_ObjInfo(),
          m_Obj(), m_ExpInfo(), m_Exp(), m_ObjVars() {}

protected:
    virtual bool LoadFunction(const Brace::FuncInfo& func, const DslData::FunctionData& funcData,
                              Brace::OperandLoadtimeInfo& resultInfo,
                              Brace::BraceApiExecutor& executor) override {
        ...
        if (num == 2) {
            ...
            } else if (nullptr != pInfo &&
                       pInfo->ObjectCategory == BRACE_OBJECT_CATEGORY_OBJ_ARRAY) {
            ...
            if (!CanAssign(elemType, elemObjTypeId, m_ExpInfo.Type, m_ExpInfo.ObjectTypeId)) {
                ...
                ss0 << "expression type dismatch the array element, " << funcData.GetId()
                    << " line " << funcData.GetLine();
                LogError(ss0.str());
                success = false;
            }
        ...
        // error
        std::stringstream ss;
        ss << "expected arraymodify(array, modify_exp), " << funcData.GetId() << " line "
           << funcData.GetLine();
        LogError(ss.str());
        return false;
    }
...
private:
    ...
    inline void DoModify(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                         const std::shared_ptr<void>& arrayWrap) const {
    ...
    inline void DoSetIter(Brace::VariableInfo& lvars, int index,
                          const std::shared_ptr<void>& val) const {
    ...
    template <>
    inline std::string DoGetVal<std::string>(Brace::VariableInfo& gvars,
                                             Brace::VariableInfo& lvars) const {
        return Brace::VarGetStr((m_ExpInfo.IsGlobal ? gvars : lvars), m_ExpInfo.Type,
                                m_ExpInfo.VarIndex);
    }
    template <>
    inline int64_t DoGetVal<int64_t>(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars) const {
        return Brace::VarGetI64((m_ExpInfo.IsGlobal ? gvars : lvars), m_ExpInfo.Type,
                                m_ExpInfo.VarIndex);
    }
    template <>
    inline double DoGetVal<double>(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars) const {
        return Brace::VarGetF64((m_ExpInfo.IsGlobal ? gvars : lvars), m_ExpInfo.Type,
                                m_ExpInfo.VarIndex);
    }
    template <>
    inline bool DoGetVal<bool>(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars) const {
        return Brace::VarGetBoolean((m_ExpInfo.IsGlobal ? gvars : lvars), m_ExpInfo.Type,
                                    m_ExpInfo.VarIndex);
    }
    template <>
    inline std::shared_ptr<void> DoGetVal<std::shared_ptr<void>>(Brace::VariableInfo& gvars,
                                                                 Brace::VariableInfo& lvars) const {
...
public:
    HashtableModifyExp(Brace::BraceScript& interpreter)
        : Brace::AbstractBraceApi(interpreter), m_ObjectCategory(INVALID_ID),
          m_IteratorIndex(INVALID_INDEX), m_ValIteratorIndex(INVALID_INDEX), m_ObjInfo(), m_Obj(),
          m_ExpInfo(), m_Exp(), m_ObjVars() {}
    ...
    virtual bool LoadFunction(const Brace::FuncInfo& func, const DslData::FunctionData& funcData,
        ...
        if (num == 2) {
            ...
            if (objTypeId >= CUSTOM_BRACE_OBJECT_TYPE_STR_STR_HASHTABLE &&
                ...
                switch (objTypeId) {
                case CUSTOM_BRACE_OBJECT_TYPE_STR_STR_HASHTABLE:
                    m_IteratorIndex = AllocVariable("$$k", Brace::BRACE_DATA_TYPE_STRING,
                                                    Brace::PREDEFINED_BRACE_OBJECT_TYPE_NOTOBJ);
                    elemType = Brace::BRACE_DATA_TYPE_STRING;
                    break;
                case CUSTOM_BRACE_OBJECT_TYPE_STR_INT_HASHTABLE:
                ...
                case CUSTOM_BRACE_OBJECT_TYPE_STR_BOOL_HASHTABLE:
                    ...
                    break;
                case CUSTOM_BRACE_OBJECT_TYPE_INT_STR_HASHTABLE:
                    m_IteratorIndex = AllocVariable("$$k", Brace::BRACE_DATA_TYPE_INT64,
                                                    Brace::PREDEFINED_BRACE_OBJECT_TYPE_NOTOBJ);
                    elemType = Brace::BRACE_DATA_TYPE_STRING;
                    break;
                case CUSTOM_BRACE_OBJECT_TYPE_INT_INT_HASHTABLE:
            ...
            if (!CanAssign(elemType, elemObjTypeId, m_ExpInfo.Type, m_ExpInfo.ObjectTypeId)) {
                std::stringstream ss0;
                ss0 << "expression type dismatch the hashtable value element, " << funcData.GetId()
                    << " line " << funcData.GetLine();
                LogError(ss0.str());
                success = false;
            }
        ...
        // error
        std::stringstream ss;
        ss << "expected hashtablemodify(hashtable, modify_exp), " << funcData.GetId() << " line "
           << funcData.GetLine();
        LogError(ss.str());
        return false;
    }
...
private:
    ...
    inline void DoModify(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                         const std::shared_ptr<void>& hashWrap) const {
    ...
    inline void SetIter(Brace::VariableInfo& lvars,
                        typename std::unordered_map<KeyT, ValT>::iterator it) const {
        DoSetIter(lvars, m_IteratorIndex, it->first);
    }
    template <typename KeyT, typename ValT>
    inline void SetValIter(Brace::VariableInfo& lvars,
                           typename std::unordered_map<KeyT, ValT>::iterator it) const {
        DoSetIter(lvars, m_ValIteratorIndex, it->second);
    }
    template <typename KeyT, typename ValT>
    inline void ChangeVal(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
                          typename std::unordered_map<KeyT, ValT>::iterator it) const {
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() != 2 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected setscriptbtncaption(index, bit_flags)," << data.GetId() << " line "
               << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        auto&& argInfo2 = argInfos[1];
        int index = static_cast<int>(Brace::VarGetI64((argInfo1.IsGlobal ? gvars : lvars),
...
public:
    SetScriptInputLabelExp(Brace::BraceScript& interpreter)
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 1 || argInfos[0].Type != Brace::BRACE_DATA_TYPE_STRING) {
            // error
            std::stringstream ss;
            ss << "expected setscriptinputlabel(string)," << data.GetId() << " line "
               << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    SetScriptBtnCaptionExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 2 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type != Brace::BRACE_DATA_TYPE_STRING) {
            // error
            std::stringstream ss;
            ss << "expected setscriptbtncaption(index, string)," << data.GetId() << " line "
               << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
protected:
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        auto&& argInfo1 = argInfos[0];
        auto&& argInfo2 = argInfos[1];
        int x = static_cast<int>(Brace::VarGetI64((argInfo1.IsGlobal ? gvars : lvars),
                                                  argInfo1.Type, argInfo1.VarIndex));
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() != 2 || argInfos[0].Type != Brace::BRACE_DATA_TYPE_STRING ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64) {
            // error
            std::stringstream ss;
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 2 || argInfos[0].Type != Brace::BRACE_DATA_TYPE_STRING ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_DOUBLE) {
            // error
            std::stringstream ss;
...
protected:
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        auto&& argInfo1 = argInfos[0];
        auto&& argInfo2 = argInfos[1];
        int m = static_cast<int>(Brace::VarGetI64((argInfo1.IsGlobal ? gvars : lvars),
                                                  argInfo1.Type, argInfo1.VarIndex));
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 2 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected keyrelease(modifier, key)," << data.GetId() << " line "
               << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() != 3 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected mousepress(x, y, button)," << data.GetId() << " line "
               << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 3 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[2].Type < Brace::BRACE_DATA_TYPE_BOOL ||
            argInfos[2].Type > Brace::BRACE_DATA_TYPE_UINT64) {
            // error
            std::stringstream ss;
            ss << "expected setbuttonstate(uint_player_index, int_button_id, bool_value),"
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        auto&& argInfo2 = argInfos[1];
        auto&& argInfo3 = argInfos[2];
        std::size_t player_index = static_cast<std::size_t>(Brace::VarGetI64(
            (argInfo1.IsGlobal ? gvars : lvars), argInfo1.Type, argInfo1.VarIndex));
        int button_id = static_cast<int>(Brace::VarGetI64((argInfo2.IsGlobal ? gvars : lvars),
                                                          argInfo2.Type, argInfo2.VarIndex));
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() != 4 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[2].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[2].Type > Brace::BRACE_DATA_TYPE_DOUBLE ||
            argInfos[3].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[3].Type > Brace::BRACE_DATA_TYPE_DOUBLE) {
            // error
            std::stringstream ss;
            ss << "expected setstickpos(uint_player_index, int_axis_id, float_x, float_y),"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        std::size_t player_index = static_cast<std::size_t>(Brace::VarGetI64(
            (argInfo1.IsGlobal ? gvars : lvars), argInfo1.Type, argInfo1.VarIndex));
        int axis_id = static_cast<int>(Brace::VarGetI64((argInfo2.IsGlobal ? gvars : lvars),
                                                        argInfo2.Type, argInfo2.VarIndex));
        float x = static_cast<float>(Brace::VarGetF64((argInfo3.IsGlobal ? gvars : lvars),
                                                      argInfo3.Type, argInfo3.VarIndex));
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() != 8 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[2].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            argInfos[3].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[3].Type > Brace::BRACE_DATA_TYPE_DOUBLE ||
            argInfos[4].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[4].Type > Brace::BRACE_DATA_TYPE_DOUBLE ||
            argInfos[5].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[5].Type > Brace::BRACE_DATA_TYPE_DOUBLE ||
            argInfos[6].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[6].Type > Brace::BRACE_DATA_TYPE_DOUBLE ||
            argInfos[7].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[7].Type > Brace::BRACE_DATA_TYPE_DOUBLE) {
            // error
            std::stringstream ss;
            ss << "expected setmotionstate(uint_player_index, uint64_delta_time, float_gyro_x, "
                  "float_gyro_y, float_gyro_z, float_accel_x, float_accel_y, float_accel_z),"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        std::size_t player_index = static_cast<std::size_t>(Brace::VarGetI64(
            (argInfo1.IsGlobal ? gvars : lvars), argInfo1.Type, argInfo1.VarIndex));
        u64 delta_time = static_cast<u64>(Brace::VarGetI64((argInfo2.IsGlobal ? gvars : lvars),
                                                           argInfo2.Type, argInfo2.VarIndex));
        float gx = static_cast<float>(Brace::VarGetF64((argInfo3.IsGlobal ? gvars : lvars),
                                                       argInfo3.Type, argInfo3.VarIndex));
        float gy = static_cast<float>(Brace::VarGetF64((argInfo4.IsGlobal ? gvars : lvars),
                                                       argInfo4.Type, argInfo4.VarIndex));
        float gz = static_cast<float>(Brace::VarGetF64((argInfo5.IsGlobal ? gvars : lvars),
                                                       argInfo5.Type, argInfo5.VarIndex));
        float ax = static_cast<float>(Brace::VarGetF64((argInfo6.IsGlobal ? gvars : lvars),
                                                       argInfo6.Type, argInfo6.VarIndex));
        float ay = static_cast<float>(Brace::VarGetF64((argInfo7.IsGlobal ? gvars : lvars),
...
static uint64_t ReadMemory(Kernel::KProcess& process, uint64_t addr, uint64_t val_size,
                           bool& result) {
    ...
    if (nullptr != g_pApiProvider) {
        ...
        switch (val_size) {
        case s_u8:
            val = sniffer.ReadMemory(process, addr, s_u8, succ);
            result = succ && result;
            break;
        case s_u16:
            val = sniffer.ReadMemory(process, addr, s_u16, succ);
            result = succ && result;
            break;
        case s_u32:
            val = sniffer.ReadMemory(process, addr, s_u32, succ);
            result = succ && result;
            break;
        case s_u64:
            val = sniffer.ReadMemory(process, addr, s_u64, succ);
            result = succ && result;
            break;
        default: {
            ...
            if (left_val_size > s_u32) {
                val += (sniffer.ReadMemory(process, addr, s_u32, succ) << lshift);
                caddr += s_u32;
                lshift += s_u32 * 8;
                left_val_size -= s_u32;
                result = succ && result;
            ...
            if (left_val_size >= s_u16) {
                val = (sniffer.ReadMemory(process, addr, s_u16, succ) << lshift);
                caddr += s_u16;
                lshift += s_u16 * 8;
                left_val_size -= s_u16;
                result = succ && result;
            ...
            if (left_val_size >= s_u8) {
                val = (sniffer.ReadMemory(process, addr, s_u8, succ) << lshift);
                caddr += s_u8;
                lshift += s_u8 * 8;
                left_val_size -= s_u8;
                result = succ && result;
...
static bool WriteMemory(Kernel::KProcess& process, uint64_t addr, uint64_t val_size, uint64_t val) {
    static const size_t s_u8 = sizeof(uint8_t);
    static const size_t s_u16 = sizeof(uint16_t);
    static const size_t s_u32 = sizeof(uint32_t);
    static const size_t s_u64 = sizeof(uint64_t);
    ...
    if (nullptr != g_pApiProvider) {
        ...
        switch (val_size) {
        case s_u8:
            succ = sniffer.WriteMemory(process, addr, s_u8, val);
            result = succ && result;
            break;
        case s_u16:
            succ = sniffer.WriteMemory(process, addr, s_u16, val);
            result = succ && result;
            break;
        case s_u32:
            succ = sniffer.WriteMemory(process, addr, s_u32, val);
            result = succ && result;
            break;
        case s_u64:
            succ = sniffer.WriteMemory(process, addr, s_u64, val);
            result = succ && result;
            break;
        default: {
            uint64_t left_val_size = val_size;
            ...
            uint64_t caddr = addr;
            if (left_val_size > s_u32) {
                succ = sniffer.WriteMemory(process, caddr, s_u32, val >> rshift);
                caddr += s_u32;
                rshift += s_u32 * 8;
                left_val_size -= s_u32;
                result = succ && result;
            ...
            if (left_val_size >= s_u16) {
                succ = sniffer.WriteMemory(process, caddr, s_u16, val >> rshift);
                caddr += s_u16;
                rshift += s_u16 * 8;
                left_val_size -= s_u16;
                result = succ && result;
            ...
            if (left_val_size >= s_u8) {
                succ = sniffer.WriteMemory(process, caddr, s_u8, val >> rshift);
                caddr += s_u8;
                rshift += s_u8 * 8;
                left_val_size -= s_u8;
                result = succ && result;
...
public:
    GetHistoryInfoCountExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
public:
    GetRollbackInfoCountExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 1 || argInfos[0].Type != Brace::BRACE_DATA_TYPE_OBJECT ||
            ...
            // error
            std::stringstream ss;
            ss << "expected setresultinfo(hashtable<:int64,MemoryModifyInfo:>)," << data.GetId()
               << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    NewMemoryModifyInfoExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        resultInfo.Type = Brace::BRACE_DATA_TYPE_OBJECT;
        resultInfo.ObjectTypeId =
            BraceScriptInterpreter::CUSTOM_BRACE_OBJECT_TYPE_CPP_MEM_MODIFY_INFO;
        resultInfo.Name = GenTempVarName();
        resultInfo.VarIndex =
            AllocVariable(resultInfo.Name, resultInfo.Type, resultInfo.ObjectTypeId);
        return true;
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() < 1 || argInfos.size() > 3 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected addtoresult(addr[, val_size, pid]), all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (nullptr != g_pApiProvider) {
            ...
            if (nullptr != pProcess) {
                uint64_t val = ReadMemory(*pProcess, addr, val_size, result);
                if (result) {
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() < 1 || argInfos.size() > 3 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected addtolast(addr[, val_size, pid]), all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
class GetProcessIdExp final : public Brace::SimpleBraceApiBase {
public:
    GetProcessIdExp(Brace::BraceScript& interpreter) : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        uint64_t progId;
        uint64_t pid;
        uint64_t base = memorySniffer.GetModuleBase(ix, addr, size, build_id, name, progId, pid);
...
protected:
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        uint64_t progId;
        uint64_t pid;
        [[maybe_unused]] uint64_t base =
            memorySniffer.GetModuleBase(ix, addr, size, build_id, name, progId, pid);
...
class GetModuleProgramIdExp final : public Brace::SimpleBraceApiBase {
public:
    GetModuleProgramIdExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 1 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected getmoduleprogid(index)," << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
        resultInfo.Type = Brace::BRACE_DATA_TYPE_UINT64;
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        Brace::VarSetUInt64(resultInfo.IsGlobal ? gvars : lvars, resultInfo.VarIndex, progId);
    }
};
class GetModuleProcessIdExp final : public Brace::SimpleBraceApiBase {
public:
    GetModuleProcessIdExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 1 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected getmodulepid(index)," << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
        resultInfo.Type = Brace::BRACE_DATA_TYPE_UINT64;
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        Brace::VarSetUInt64(resultInfo.IsGlobal ? gvars : lvars, resultInfo.VarIndex, pid);
    }
};
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() < 2 || argInfos.size() > 4 ||
            argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            (argInfos.size() == 3 && (argInfos[2].Type < Brace::BRACE_DATA_TYPE_BOOL ||
                                      argInfos[2].Type > Brace::BRACE_DATA_TYPE_UINT64)) ||
            (argInfos.size() == 4 && (argInfos[3].Type < Brace::BRACE_DATA_TYPE_INT8 ||
                                      argInfos[3].Type > Brace::BRACE_DATA_TYPE_UINT64))) {
            // error
            std::stringstream ss;
            ss << "expected markmemdebug(uint64 addr, uint64 size[, bool debug[, uint64 pid]]),"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (argInfos.size() >= 3) {
            auto&& argInfo3 = argInfos[2];
            debug = Brace::VarGetBoolean(argInfo3.IsGlobal ? gvars : lvars, argInfo3.Type,
                                         argInfo3.VarIndex);
        }
        if (argInfos.size() == 4) {
            auto&& argInfo4 = argInfos[3];
            pid = Brace::VarGetBoolean(argInfo4.IsGlobal ? gvars : lvars, argInfo4.Type,
                                       argInfo4.VarIndex);
        }
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() < 2 || argInfos.size() > 5 ||
            argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected addsniffing(uint64 addr, uint64 size[, uint64 step, uint64 val, uint64 "
                  "pid]),"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
...
public:
    CmdAddSniffingFromSearchExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               ...
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() < 1 || argInfos.size() > 2 ||
            argInfos[0].Type != Brace::BRACE_DATA_TYPE_OBJECT ||
            argInfos[0].ObjectTypeId != CUSTOM_BRACE_OBJECT_TYPE_INT_ARRAY) {
            // error
            std::stringstream ss;
            ss << "expected addsniffingfromsearch(find_vals)," << data.GetId() << " line "
               << data.GetLine();
            LogError(ss.str());
            return false;
        }
                ...
                if (findVals.find(val) != findVals.end()) {
                    ...
                    if (pqueue.size() == findVals.size()) {
                        ...
                        if (addr - stAddr <= range) {
                            sniffer.AddSniffing(pid, stAddr, addr + val_size - stAddr, step, 0);
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() < 2 || argInfos.size() > 4 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected showmem(uint64 addr, uint64 size[, uint64 step, uint64 pid]),"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() < 5 || argInfos.size() > 7 ||
            argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            ...
            argInfos[3].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[3].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[4].Type != Brace::BRACE_DATA_TYPE_OBJECT ||
            argInfos[4].ObjectTypeId != CUSTOM_BRACE_OBJECT_TYPE_INT_ARRAY ||
            (argInfos.size() >= 6 && (argInfos[5].Type < Brace::BRACE_DATA_TYPE_INT8 ||
                                      argInfos[5].Type > Brace::BRACE_DATA_TYPE_UINT64)) ||
            (argInfos.size() == 7 && (argInfos[6].Type < Brace::BRACE_DATA_TYPE_INT8 ||
                                      argInfos[6].Type > Brace::BRACE_DATA_TYPE_UINT64))) {
            // error
            std::stringstream ss;
            ss << "expected findmemory(start, size, step, range, find_vals[, val_size, pid]), all type "
                  "is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() < 5 || argInfos.size() > 8 ||
            argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            ...
            (argInfos.size() >= 7 && (argInfos[6].Type < Brace::BRACE_DATA_TYPE_INT8 ||
                                      argInfos[6].Type > Brace::BRACE_DATA_TYPE_UINT64)) ||
            (argInfos.size() == 8 && (argInfos[7].Type < Brace::BRACE_DATA_TYPE_INT8 ||
                                      argInfos[7].Type > Brace::BRACE_DATA_TYPE_UINT64))) {
            // error
            std::stringstream ss;
            ss << "expected searchmemory(start, size, step, range, find_vals[, val_size, "
                  "max_count, pid]), all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() < 1 || argInfos.size() > 3 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected readmemory(addr[, val_size, pid]), all type is integer," << data.GetId()
               << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (nullptr != g_pApiProvider) {
            ...
            if (nullptr != pProcess) {
                bool result = false;
                val = ReadMemory(*pProcess, addr, val_size, result);
                if (!result) {
                    std::stringstream ss;
                    ss << "read addr:" << std::hex << addr << " size:" << std::dec << val_size
                       << " failed.";
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() < 2 || argInfos.size() > 4 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected writememory(addr, val[, val_size, pid]), all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (nullptr != g_pApiProvider) {
            ...
            auto* pProcess = sniffer.GetProcess(pid);
            if (nullptr != pProcess) {
                result = WriteMemory(*pProcess, addr, val_size, val);
                if (!result) {
                    std::stringstream ss;
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() < 3 || argInfos.size() > 4 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dumpmemory(uint64 addr, uint64 size, string file_path[, pid])," << data.GetId()
               << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (nullptr != g_pApiProvider) {
            ...
            auto* pProcess = sniffer.GetProcess(pid);
            if (nullptr != pProcess) {
                std::ofstream file(get_absolutely_path(file_path),
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 2 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected addloginst(mask, value), all type is integer," << data.GetId()
               << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 3 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected replacesourceshader(uint64 shader_hash, int stage, string file_path),"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    ReplaceSpirvShaderExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 3 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected replacespirvshader(uint64 shader_hash, int stage, string file_path),"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    DmntFileExp(Brace::BraceScript& interpreter)
        : Brace::AbstractBraceApi(interpreter), m_Args(), m_ArgInfos(), m_Statements() {}

protected:
    virtual bool LoadFunction([[maybe_unused]] const Brace::FuncInfo& curFunc,
                              const DslData::FunctionData& funcData,
                              [[maybe_unused]] Brace::OperandLoadtimeInfo& resultInfo,
                              Brace::BraceApiExecutor& executor) override {
        ...
        if (funcData.IsHighOrder()) {
            ...
            if (num == 2 || num == 3 || num == 4) {
                ...
                if (argLoadInfos[0].Type == Brace::BRACE_DATA_TYPE_STRING &&
                    ...
                    (num == 2 ||
                     (num >= 3 && argLoadInfos[2].Type == Brace::BRACE_DATA_TYPE_STRING)) &&
                    (num == 2 ||
                     (num == 4 && argLoadInfos[3].Type == Brace::BRACE_DATA_TYPE_STRING))) {
                    PushBlock();
        ...
        // error
        std::stringstream ss;
        ss << "expected 'dmnt_file(name, module[, file_dir[, build_id]]){...};', line "
           << funcData.GetLine();
        return false;
    }
    virtual bool LoadStatement([[maybe_unused]] const Brace::FuncInfo& curFunc,
                               const DslData::StatementData& data,
                               [[maybe_unused]] Brace::OperandLoadtimeInfo& resultInfo,
                               Brace::BraceApiExecutor& executor) override {
        ...
        if (data.GetFunctionNum() == 2) {
            ...
            if (!first->HaveStatement() && !first->HaveExternScript()) {
                ...
                if (nullptr != secondVal ||
                    (nullptr != secondFunc && secondFunc->HaveId() &&
                     !secondFunc->HaveStatement() && !secondFunc->HaveExternScript())) {
                    if (first->GetParamNum() > 0) {
                        ...
                        if (nullptr != pCallData) {
                            ...
                            if (num == 2 || num == 3 || num == 4) {
                                ...
                                if (argLoadInfos[0].Type == Brace::BRACE_DATA_TYPE_STRING &&
                                    argLoadInfos[1].Type == Brace::BRACE_DATA_TYPE_STRING &&
                                    (num == 2 || (num >= 3 && argLoadInfos[2].Type ==
                                                                  Brace::BRACE_DATA_TYPE_STRING)) &&
                                    (num == 2 || (num == 4 && argLoadInfos[3].Type ==
                                                                  Brace::BRACE_DATA_TYPE_STRING))) {
                                    PushBlock();
                                    Brace::OperandLoadtimeInfo argLoadInfo;
        ...
        // error
        std::stringstream ss;
        ss << "expected 'dmnt_file(name, module[, file_dir[, build_id]]) func(...);', line "
           << data.GetLine();
        return false;
    }
...
private:
    int Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars) const {
        ...
        for (int ix = 0; ix < sniffer.GetModuleCount(); ++ix) {
            ...
            uint64_t progId;
            uint64_t pid;
            uint64_t base = sniffer.GetModuleBase(ix, addr, size, build_id, mname, progId, pid);
            if (mname == _module) {
        ...
        ss.str("");
        ss << std::uppercase;
        ss << "{ " << name << " " << bid << " [" << std::setfill('0')
           << std::setw(sizeof(uint64_t) * 2) << std::hex << title_id << "] }" << std::endl;
...
public:
    DmntIfExp(Brace::BraceScript& interpreter)
        : Brace::AbstractBraceApi(interpreter), m_Clauses() {}
...
protected:
    ...
    virtual bool LoadStatement([[maybe_unused]] const Brace::FuncInfo& curFunc,
        ...
        for (int ix = 0; ix < data.GetFunctionNum(); ++ix) {
            ...
            if (fData->GetId() == "dmnt_if" || fData->GetId() == "elseif" ||
                fData->GetId() == "elif") {
                Clause item;
                if (fData->IsHighOrder() && fData->GetLowerOrderFunction().GetParamNum() > 0) {
                    ...
                    // error
                    std::stringstream ss;
                    ss << "expected 'dmnt_if(exp){...}elseif/elif(exp){...}else{...};', line "
                       << data.GetLine();
                    LogError(ss.str());
                }
                PushBlock();
...
public:
    DmntLoopExp(Brace::BraceScript& interpreter)
        : Brace::AbstractBraceApi(interpreter), m_Args(), m_ArgInfos(), m_Statements() {}
    ...
    virtual bool LoadFunction([[maybe_unused]] const Brace::FuncInfo& curFunc,
        ...
        if (funcData.IsHighOrder()) {
            ...
            if (num == 2) {
                ...
                if (argLoadInfos[0].Type >= Brace::BRACE_DATA_TYPE_INT8 &&
                    argLoadInfos[0].Type <= Brace::BRACE_DATA_TYPE_UINT64 &&
                    argLoadInfos[1].Type >= Brace::BRACE_DATA_TYPE_INT8 &&
                    argLoadInfos[1].Type <= Brace::BRACE_DATA_TYPE_UINT64) {
                    PushBlock();
                    for (int ix = 0; ix < funcData.GetParamNum(); ++ix) {
                        Brace::OperandLoadtimeInfo argLoadInfo;
                        auto statement = LoadHelper(*funcData.GetParam(ix), argLoadInfo);
...
private:
    int Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars) const {
        ...
        auto&& argInfo1 = m_ArgInfos[0];
        auto&& argInfo2 = m_ArgInfos[1];
        int reg = static_cast<int>(Brace::VarGetI64((argInfo1.IsGlobal ? gvars : lvars),
                                                    argInfo1.Type, argInfo1.VarIndex));
        int ct = static_cast<int>(Brace::VarGetI64((argInfo2.IsGlobal ? gvars : lvars),
                                                   argInfo2.Type, argInfo2.VarIndex));
        // gen begin
        // 300R0000 VVVVVVVV
    ...
    virtual bool LoadFunction([[maybe_unused]] const Brace::FuncInfo& curFunc,
        ...
        if (funcData.HaveParam() && funcData.GetParamNum() == 1) {
            ...
            std::unordered_map<std::string, uint32_t> key2masks = {
                ...
                {"LS", 0x10},      {"RS", 0x20},      {"L", 0x40},       {"R", 0x80},
                {"ZL", 0x100},     {"ZR", 0x200},     {"Plus", 0x400},   {"Minus", 0x800},
                {"Left", 0x1000},  {"Up", 0x2000},    {"Right", 0x4000}, {"Down", 0x8000},
                {"LSL", 0x10000},  {"LSU", 0x20000},  {"LSR", 0x40000},  {"LSD", 0x80000},
                {"RSL", 0x100000}, {"RSU", 0x200000}, {"RSR", 0x400000}, {"RSD", 0x800000},
                {"SL", 0x1000000}, {"SR", 0x2000000},
            };
            std::string varId;
            ...
            if (it == key2masks.end()) {
                // error
                std::stringstream ss;
                ss << "expected 'dmnt_key(key)' "
                      "key:A|B|X|Y|LS|RS|L|R|ZL|ZR|Plus|Minus|Left|Up|Right|Down|LSL|LSU|LSR|LSD|"
                      "RSL|RSU|RSR|RSD|SL|SR, line "
                   << funcData.GetLine();
                return false;
            } else {
            ...
            if (nullptr != info) {
                ...
                resultInfo.VarIndex = AllocConst(DslData::ValueData::VALUE_TYPE_NUM, varId,
                                                 resultInfo.Type, resultInfo.ObjectTypeId);
                resultInfo.IsGlobal = true;
                resultInfo.IsTempVar = false;
                resultInfo.IsConst = true;
                resultInfo.Name = varId;
        ...
        std::stringstream ss;
        ss << "expected 'dmnt_key(key)' "
              "key:A|B|X|Y|LS|RS|L|R|ZL|ZR|Plus|Minus|Left|Up|Right|Down|LSL|LSU|LSR|LSD|RSL|RSU|"
              "RSR|RSD|SL|SR, line "
           << funcData.GetLine();
        return false;
    }
    ...
    virtual bool LoadFunction([[maybe_unused]] const Brace::FuncInfo& curFunc,
        ...
        if (funcData.HaveParam() && funcData.GetParamNum() == 1) {
            ...
            if (name == "main") {
                ...
                // error
                std::stringstream ss;
                ss << "expected 'dmnt_region(mem_region)' mem_region:main|heap|alias|aslr, line "
                   << funcData.GetLine();
                return false;
            }
            auto* info = GetConstInfo(DslData::ValueData::VALUE_TYPE_NUM, varId);
    ...
    virtual bool LoadFunction([[maybe_unused]] const Brace::FuncInfo& curFunc,
        ...
        if (funcData.HaveParam() && funcData.GetParamNum() == 1) {
            ...
            std::unordered_map<std::string, uint32_t> key2vals = {
                {"no_offset", 0},           {"offset_reg", 1},
                {"offset_fixed", 2},        {"region_and_base", 3},
                {"region_and_relative", 4}, {"region_and_relative_and_offset", 5},
            };
            std::string varId;
            ...
            if (it == key2vals.end()) {
                // error
                std::stringstream ss;
                ss << "expected 'dmnt_offset(name)' "
                      "name:no_offset|offset_reg|offset_fixed|region_and_base|region_and_relative|"
                      "region_and_relative_and_offset, line "
                   << funcData.GetLine();
                return false;
            } else {
                varId = std::to_string(it->second);
        ...
        std::stringstream ss;
        ss << "expected 'dmnt_offset(name)' "
              "name:no_offset|offset_reg|offset_fixed|region_and_base|region_and_relative|region_"
              "and_relative_and_offset, line "
           << funcData.GetLine();
        return false;
    }
};
...
protected:
    virtual bool LoadFunction([[maybe_unused]] const Brace::FuncInfo& curFunc,
        ...
        if (funcData.HaveParam() && funcData.GetParamNum() == 1) {
            const std::string& name = funcData.GetParamId(0);
            std::unordered_map<std::string, uint32_t> key2vals = {
                {"mem_and_relative", 0},  {"mem_and_offset", 1},   {"reg_and_relative", 2},
                {"reg_and_offset", 3},    {"static_value", 4},     {"register_value", 4},
                {"reg_other", 5},         {"restore_register", 0}, {"save_register", 1},
                {"clear_saved_value", 2}, {"clear_register", 3},
            };
            std::string varId;
            auto&& it = key2vals.find(name);
            if (it == key2vals.end()) {
                // error
                std::stringstream ss;
                ss << "expected 'dmnt_operand(name)' "
                      "name:mem_and_relative|mem_and_offset|reg_and_relative|reg_and_offset|static_"
                      "value|register_value|reg_other|restore_register|save_register|clear_saved_"
                      "value|clear_register, line "
                   << funcData.GetLine();
                return false;
            } else {
                varId = std::to_string(it->second);
        ...
        std::stringstream ss;
        ss << "expected 'dmnt_operand(name)' "
              "name:mem_and_relative|mem_and_offset|reg_and_relative|reg_and_offset|static_value|"
              "register_value|reg_other|restore_register|save_register|clear_saved_value|clear_"
              "register, line "
           << funcData.GetLine();
        return false;
    }
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 3 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_calc_offset(offset, addr, region), all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() < 2 || argInfos.size() > 4 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_read_mem(val, addr[, val_size, pid]), all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    DmntStoreValueToAddrExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if (argInfos.size() != 5 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[2].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_store_v2a(mem_width, mem_region, reg, offset, val), all type is "
                  "integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
...
public:
    DmntConditionExp(Brace::BraceScript& interpreter, const std::string& op)
        : Brace::SimpleBraceApiBase(interpreter), m_Operator(op) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 4 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_xxx(mem_width, mem_region, offset, val), all type is integer, "
                  "xxx:gt|ge|lt|le|eq|ne,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    DmntLoadRegWithValueExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 2 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_load_v2r(reg, val), all type is integer," << data.GetId()
               << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    DmntLoadRegWithMemoryExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if ((argInfos.size() != 3 && argInfos.size() != 4) ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_load_m2r(mem_width[, mem_region], reg, offset), all type is "
                  "integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
...
public:
    DmntStoreValueToMemoryExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if ((argInfos.size() != 4 && argInfos.size() != 5) ||
            argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_store_v2m(mem_width, mem_reg, reg_inc_1or0, val[, offset_reg]), "
                  "all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
...
public:
    DmntLegacyArithExp(Brace::BraceScript& interpreter, const std::string& op)
        : Brace::SimpleBraceApiBase(interpreter), m_Operator(op) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 3 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_legacy_xxx(mem_width, reg, val), all type is integer, "
                  "xxx:add|sub|mul|lshift|rshift,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        for (auto&& argInfo : argInfos) {
            if (argInfo.Type < Brace::BRACE_DATA_TYPE_INT8 ||
                ...
                // error
                std::stringstream ss;
                ss << "expected dmnt_keypress(key1,key2,...); all type is integer, key can get by "
                      "dmnt_key(const),"
                   << data.GetId() << " line " << data.GetLine();
                LogError(ss.str());
                return false;
...
public:
    DmntArithExp(Brace::BraceScript& interpreter, const std::string& op)
        : Brace::SimpleBraceApiBase(interpreter), m_Operator(op) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if ((argInfos.size() != 4 && argInfos.size() != 5) ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_xxx(mem_width, result_reg, lhs_reg, rhs[, rhs_is_val_1or0]), all "
                  "type is integer, xxx:add|sub|mul|lshift|rshift|and|or|not|xor|mov,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        if (argInfos.size() == 5) {
            auto&& argInfo5 = argInfos[4];
            rhs_is_val_1or0 = static_cast<int>(Brace::VarGetI64(argInfo5.IsGlobal ? gvars : lvars,
                                                                argInfo5.Type, argInfo5.VarIndex));
        }
...
public:
    DmntStoreRegToMemoryExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
                               const std::vector<Brace::OperandLoadtimeInfo>& argInfos,
                               Brace::OperandLoadtimeInfo& resultInfo) override {
        if ((argInfos.size() != 4 && argInfos.size() != 6 && argInfos.size() != 7) ||
            argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[0].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            argInfos[1].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            argInfos[1].Type > Brace::BRACE_DATA_TYPE_UINT64 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_store_r2m(mem_width, src_reg, mem_reg, reg_inc_1or0,[ "
                  "offset_type, offset_or_reg_or_region[, offset]]), all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    DmntRegCondExp(Brace::BraceScript& interpreter, const std::string& op)
        : Brace::SimpleBraceApiBase(interpreter), m_Operator(op) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if ((argInfos.size() != 4 && argInfos.size() != 5) ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_reg_xxx(mem_width, src_reg, opd_type, val1[, val2]), all type is "
                  "integer, xxx:gt|ge|lt|le|eq|ne,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    DmntRegSaveRestoreExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 3 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_reg_sr(dest_reg, src_reg, opd_type), all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
public:
    DmntRegSaveRestoreWithMaskExp(Brace::BraceScript& interpreter)
        : Brace::SimpleBraceApiBase(interpreter) {}
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 2 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_reg_sr_mask(opd_type, mask), all type is integer," << data.GetId()
               << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if (argInfos.size() != 2 || argInfos[0].Type < Brace::BRACE_DATA_TYPE_INT8 ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_reg_rw(static_reg_index, reg), all type is integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
        }
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        auto&& argInfo1 = argInfos[0];
        auto&& argInfo2 = argInfos[1];
        int static_reg_index = static_cast<int>(
            Brace::VarGetI64(argInfo1.IsGlobal ? gvars : lvars, argInfo1.Type, argInfo1.VarIndex));
...
protected:
    virtual bool TypeInference(const Brace::FuncInfo& func, const DslData::FunctionData& data,
        ...
        if ((argInfos.size() != 4 && argInfos.size() != 5) ||
            ...
            // error
            std::stringstream ss;
            ss << "expected dmnt_debug(mem_width, log_id, opd_type, val1[, val2]), all type is "
                  "integer,"
               << data.GetId() << " line " << data.GetLine();
            LogError(ss.str());
            return false;
    ...
    virtual void Execute(Brace::VariableInfo& gvars, Brace::VariableInfo& lvars,
        ...
        auto&& argInfo3 = argInfos[2];
        auto&& argInfo4 = argInfos[3];
        int mem_width = static_cast<int>(
            Brace::VarGetI64(argInfo1.IsGlobal ? gvars : lvars, argInfo1.Type, argInfo1.VarIndex));
        int log_id = static_cast<int>(
            Brace::VarGetI64(argInfo2.IsGlobal ? gvars : lvars, argInfo2.Type, argInfo2.VarIndex));
        int opd_type = static_cast<int>(
            Brace::VarGetI64(argInfo3.IsGlobal ? gvars : lvars, argInfo3.Type, argInfo3.VarIndex));
        uint64_t val1 = static_cast<uint64_t>(
...
inline void BraceScriptManager::InitBraceScript(Brace::BraceScript*& pBraceScript,
    ...
    pBraceScript->RegisterApi("membercall", "object member call",
                              new Brace::BraceApiFactory<MemberCallExp>());
    pBraceScript->RegisterApi("memberset", "object property set",
                              new Brace::BraceApiFactory<MemberSetExp>());
    pBraceScript->RegisterApi("memberget", "object property get",
                              new Brace::BraceApiFactory<MemberGetExp>());
    pBraceScript->RegisterApi("collectioncall", "collection member call",
                              new Brace::BraceApiFactory<CollectionCallExp>());
    pBraceScript->RegisterApi("collectionset", "collection property set",
                              new Brace::BraceApiFactory<CollectionSetExp>());
    pBraceScript->RegisterApi("collectionget", "collection property get",
                              new Brace::BraceApiFactory<CollectionGetExp>());
    pBraceScript->RegisterApi(
        "linq",
        "objs.where(args) or objs.orderby(args) or objs.orderbydesc(args) or objs.top(args) or "
        "linq(objs,method_str,arg1,arg2,...) linq expression",
        new Brace::BraceApiFactory<LinqExp>());
    pBraceScript->RegisterApi("select",
                              "select(fields)top(10)from(objlist)where(exp)oderby(exps)groupby("
                              "exps)having(exp){statements;}; statement",
                              new Brace::BraceApiFactory<SelectExp>());
    pBraceScript->RegisterApi("lambda",
                              "lambda expression, (args) => {...} or (args)int => {...} or "
                              "[...](args) => {...} or [...](args)int => {...} lambda expression",
                              new Brace::BraceApiFactory<LambdaExp>());
    pBraceScript->RegisterApi("array",
                              "[v1,v2,...] or array(v1,v2,...) or array<:type:>(v1,v2,...) object",
                              new Brace::BraceApiFactory<ArrayExp>());
    pBraceScript->RegisterApi(
        "hashtable",
        "{k1=>v1,k2=>v2,...} or {k1:v1,k2:v2,...} or hashtable(k1=>v1,k2=>v2,...) or "
        "hashtable(k1:v1,k2:v2,...) or hashtable<:key_type,val_type:>(k1=>v1,k2=>v2,...) or "
        "hashtable<:key_type,val_type:>(k1:v1,k2:v2,...) object",
        new Brace::BraceApiFactory<HashtableExp>());
    pBraceScript->RegisterApi(
        "looplist", "looplist(list)func(args); or looplist(list){...}; statement, iterator is $$",
        new Brace::BraceApiFactory<LoopListExp>());
    pBraceScript->RegisterApi("cast", "cast(exp,type) api", new Brace::BraceApiFactory<CastExp>());
    pBraceScript->RegisterApi("typetag", "typetag(type) or typetag(exp) api",
                              new Brace::BraceApiFactory<TypeTagExp>());
    pBraceScript->RegisterApi("typeid", "typeid(type) or typeid(exp) api",
                              new Brace::BraceApiFactory<TypeIdExp>());
    pBraceScript->RegisterApi("objtypeid", "objtypeid(type) or objtypeid(exp) api",
                              new Brace::BraceApiFactory<ObjTypeIdExp>());
    pBraceScript->RegisterApi("getobjtypename", "getobjtypename(objtypeid) api",
                              new Brace::BraceApiFactory<GetObjTypeNameExp>());
    pBraceScript->RegisterApi("getobjcategory", "getobjcategory(objtypeid) api",
                              new Brace::BraceApiFactory<GetObjCategoryExp>());
    pBraceScript->RegisterApi("gettypeparamcount", "gettypeparamcount(objtypeid) api",
                              new Brace::BraceApiFactory<GetTypeParamCountExp>());
    pBraceScript->RegisterApi("gettypeparamtype", "gettypeparamtype(objtypeid,index) api",
                              new Brace::BraceApiFactory<GetTypeParamTypeExp>());
    pBraceScript->RegisterApi("gettypeparamobjtypeid", "gettypeparamobjtypeid(objtypeid,index) api",
                              new Brace::BraceApiFactory<GetTypeParamObjTypeIdExp>());
    pBraceScript->RegisterApi("swap", "swap(var1,var2) api", new Brace::BraceApiFactory<SwapExp>());
    pBraceScript->RegisterApi("struct", "struct(name){a:int32;b:int32;...}, define struct",
                              new Brace::BraceApiFactory<StructExp>());
    pBraceScript->RegisterApi("newstruct", "newstruct(struct_type) api",
                              new Brace::BraceApiFactory<NewStructExp>());
    pBraceScript->RegisterApi("reinterpretas", "reinterpret_cast(uint,struct_type) api",
                              new Brace::BraceApiFactory<ReInterpretAsExp>());
    ...
    if (isCallback) {
        pBraceScript->RegisterApi("wait", "wait(ms,...) api",
                                  new Brace::BraceApiFactoryWithArgs<WaitExp, bool>(true));
    } else {
        pBraceScript->RegisterApi("wait", "wait(ms,...) api",
                                  new Brace::BraceApiFactoryWithArgs<WaitExp, bool>(false));
        pBraceScript->RegisterApi("waituntilquit", "waituntilquit() api",
    ...
    pBraceScript->RegisterApi("char2int", "char2int(str) api",
                              new Brace::BraceApiFactory<Char2IntExp>());
    pBraceScript->RegisterApi("int2hex", "int2hex(val) api",
                              new Brace::BraceApiFactory<Int2HexExp>());
    pBraceScript->RegisterApi("hex2int", "hex2int(str) api",
                              new Brace::BraceApiFactory<Hex2IntExp>());
    pBraceScript->RegisterApi("int2str", "int2str(val) api",
                              new Brace::BraceApiFactory<Int2StrExp>());
    pBraceScript->RegisterApi("str2int", "str2int(str) api",
                              new Brace::BraceApiFactory<Str2IntExp>());
    pBraceScript->RegisterApi("float2str", "float2str(num[,precise]) api",
                              new Brace::BraceApiFactory<Float2StrExp>());
    pBraceScript->RegisterApi("str2float", "str2float(str) api",
                              new Brace::BraceApiFactory<Str2FloatExp>());
    pBraceScript->RegisterApi("strconcat", "strconcat(str,str,...) api",
                              new Brace::BraceApiFactory<StrConcatExp>());
    pBraceScript->RegisterApi("strcontainsone", "strcontainsone(str,str,...) api",
                              new Brace::BraceApiFactory<StrContainsOneExp>());
    pBraceScript->RegisterApi("strcontainsall", "strcontainsall(str,str,...) api",
                              new Brace::BraceApiFactory<StrContainsAllExp>());
    pBraceScript->RegisterApi("strindexof", "strindexof(str,str[,index]) api",
                              new Brace::BraceApiFactory<StrIndexOfExp>());
    pBraceScript->RegisterApi("strlastindexof", "strlastindexof(str,str[,index]) api",
                              new Brace::BraceApiFactory<StrLastIndexOfExp>());
    pBraceScript->RegisterApi("strlen", "strlen(str) api", new Brace::BraceApiFactory<StrLenExp>());
    pBraceScript->RegisterApi("substr", "substr(str,pos[,count]) api",
                              new Brace::BraceApiFactory<SubStrExp>());
    pBraceScript->RegisterApi("strreplace", "strreplace(str,str,str) api",
                              new Brace::BraceApiFactory<StrReplaceExp>());
    pBraceScript->RegisterApi("strsplit", "strsplit(str,str) api",
                              new Brace::BraceApiFactory<StrSplitExp>());
    pBraceScript->RegisterApi("strjoin", "strjoin(array<:string:>,str) api",
                              new Brace::BraceApiFactory<StrJoinExp>());
    pBraceScript->RegisterApi("csvecho", "csvecho(args) api",
    ...
    pBraceScript->RegisterApi("loadfile", "loadfile(file) api",
                              new Brace::BraceApiFactory<LoadFileExp>());
    pBraceScript->RegisterApi("savefile", "savefile(str,file) api",
                              new Brace::BraceApiFactory<SaveFileExp>());
    pBraceScript->RegisterApi("loadfiletoarray", "loadfiletoarray(file[,typetag(arr_type)]) api",
                              new Brace::BraceApiFactory<LoadFileToArrayExp>());
    pBraceScript->RegisterApi("savearraytofile", "savearraytofile(arr,file) api",
                              new Brace::BraceApiFactory<SaveArrayToFileExp>());

    pBraceScript->RegisterApi("savehashtable", "savehashtable(hashtable,file) api",
                              new Brace::BraceApiFactory<SaveHashtableExp>());
    pBraceScript->RegisterApi("loadhashtable", "loadhashtable(file[,typetag(hashtable_type)]) api",
                              new Brace::BraceApiFactory<LoadHashtableExp>());
    pBraceScript->RegisterApi("calcnewitems", "calcnewitems(hashtable1,hashtable2) api",
                              new Brace::BraceApiFactory<CalcNewItemsExp>());
    pBraceScript->RegisterApi("calcsameitems", "calcsameitems(hashtable1,hashtable2) api",
                              new Brace::BraceApiFactory<CalcSameItemsExp>());
    pBraceScript->RegisterApi("calcitemsunion", "calcitemsunion(hashtable1,hashtable2) api",
                              new Brace::BraceApiFactory<CalcItemsUnionExp>());
    pBraceScript->RegisterApi("itemsadd", "itemsadd(hashtable1,hashtable2) api",
                              new Brace::BraceApiFactory<ItemsAddExp>());
    pBraceScript->RegisterApi("itemssub", "itemssub(hashtable1,hashtable2) api",
                              new Brace::BraceApiFactory<ItemsSubExp>());
    pBraceScript->RegisterApi("itemsmul", "itemsmul(hashtable1,hashtable2) api",
                              new Brace::BraceApiFactory<ItemsMulExp>());
    pBraceScript->RegisterApi("itemsdiv", "itemsdiv(hashtable1,hashtable2) api",
                              new Brace::BraceApiFactory<ItemsDivExp>());

    pBraceScript->RegisterApi("arrayadd", "arrayadd(arr1,arr2) api",
                              new Brace::BraceApiFactory<ArrayAddExp>());
    pBraceScript->RegisterApi("arraysub", "arraysub(arr1,arr2) api",
                              new Brace::BraceApiFactory<ArraySubExp>());
    pBraceScript->RegisterApi("arraymul", "arraymul(arr1,arr2) api",
                              new Brace::BraceApiFactory<ArrayMulExp>());
    pBraceScript->RegisterApi("arraydiv", "arraydiv(arr1,arr2) api",
                              new Brace::BraceApiFactory<ArrayDivExp>());

    pBraceScript->RegisterApi("arraymodify", "arraymodify(array,modify_exp) api, iterator is $$",
                              new Brace::BraceApiFactory<ArrayModifyExp>());
    pBraceScript->RegisterApi("hashtablemodify",
                              "hashtablemodify(hashtable,modify_exp) api, iterator is $$k and $$v",
                              new Brace::BraceApiFactory<HashtableModifyExp>());
    ...
    pBraceScript->RegisterApi("hypot", "hypot(x,y) or hypot(x,y,z) api",
                              new Brace::BraceApiFactory<HypotExp>());
    ...
    pBraceScript->RegisterApi("deg2rad", "deg2rad(number) api",
                              new Brace::BraceApiFactory<Deg2RadExp>());
    pBraceScript->RegisterApi("rad2deg", "rad2deg(number) api",
                              new Brace::BraceApiFactory<Rad2DegExp>());
    pBraceScript->RegisterApi("randint",
                              "randint() or randint(max_num) or randint(min_num,max_num) api",
                              new Brace::BraceApiFactory<RandIntExp>());
    pBraceScript->RegisterApi("randfloat",
                              "randfloat() or randfloat(max_num) or randfloat(min_num,max_num) api",
                              new Brace::BraceApiFactory<RandFloatExp>());
    ...
    pBraceScript->RegisterApi("devsq", "devsq(number,...) api",
                              new Brace::BraceApiFactory<DevSqExp>());

    pBraceScript->RegisterApi("arraymax", "arraymax(int_array) or arraymax(float_array) api",
                              new Brace::BraceApiFactory<ArrayMaxExp>());
    pBraceScript->RegisterApi("arraymin", "arraymin(int_array) or arraymin(float_array) api",
                              new Brace::BraceApiFactory<ArrayMinExp>());
    pBraceScript->RegisterApi("arraysum", "arraysum(int_array) or arraysum(float_array) api",
                              new Brace::BraceApiFactory<ArraySumExp>());
    pBraceScript->RegisterApi("arrayavg", "arrayavg(int_array) or arrayavg(float_array) api",
                              new Brace::BraceApiFactory<ArrayAvgExp>());
    pBraceScript->RegisterApi("arraydevsq", "arraydevsq(int_array) or arraydevsq(float_array) api",
                              new Brace::BraceApiFactory<ArrayDevSqExp>());

    pBraceScript->RegisterApi("hashtablemax",
                              "hashtablemax(int_int_hash) or hashtablemax(str_int_hash) or "
                              "hashtablemax(int_float_hash) or hashtablemax(str_float_hash) api",
                              new Brace::BraceApiFactory<HashtableMaxExp>());
    pBraceScript->RegisterApi("hashtablemin",
                              "hashtablemin(int_int_hash) or hashtablemin(str_int_hash) or "
                              "hashtablemin(int_float_hash) or hashtablemin(str_float_hash) api",
                              new Brace::BraceApiFactory<HashtableMinExp>());
    pBraceScript->RegisterApi("hashtablesum",
                              "hashtablesum(int_int_hash) or hashtablesum(str_int_hash) or "
                              "hashtablesum(int_float_hash) or hashtablesum(str_float_hash) api",
                              new Brace::BraceApiFactory<HashtableSumExp>());
    pBraceScript->RegisterApi("hashtableavg",
                              "hashtableavg(int_int_hash) or hashtableavg(str_int_hash) or "
                              "hashtableavg(int_float_hash) or hashtableavg(str_float_hash) api",
                              new Brace::BraceApiFactory<HashtableAvgExp>());
    pBraceScript->RegisterApi(
        "hashtabledevsq",
        "hashtabledevsq(int_int_hash) or hashtabledevsq(str_int_hash) or "
        "hashtabledevsq(int_float_hash) or hashtabledevsq(str_float_hash) api",
        new Brace::BraceApiFactory<HashtableDevSqExp>());

    pBraceScript->RegisterApi(
        "linearregression",
        "linearregression(array<:array<:double:>:>,array<:double:>) or "
        "linearregression(array<:array<:double:>:>,array<:double:>,bool_debug) api",
        new Brace::BraceApiFactory<LinearRegressionExp>());

    //
    pBraceScript->RegisterApi("getexepath", "getexepath() api",
                              new Brace::BraceApiFactory<GetExePathExp>());
    ...
    pBraceScript->RegisterApi("showui", "showui(index,bit_flags) api",
                              new Brace::BraceApiFactory<ShowUiExp>());
    pBraceScript->RegisterApi("getscriptinput", "getscriptinput() api",
                              new Brace::BraceApiFactory<GetScriptInputExp>());
    pBraceScript->RegisterApi("setscriptinputlabel", "setscriptinputlabel(str) api",
                              new Brace::BraceApiFactory<SetScriptInputLabelExp>());
    pBraceScript->RegisterApi("setscriptbtncaption", "setscriptbtncaption(index,str) api",
                              new Brace::BraceApiFactory<SetScriptBtnCaptionExp>());

    pBraceScript->RegisterApi("getpixel", "getpixel(x,y) api",
                              new Brace::BraceApiFactory<GetPixelExp>());
    pBraceScript->RegisterApi("getcursorx", "getcursorx() api",
                              new Brace::BraceApiFactory<GetCursorXExp>());
    pBraceScript->RegisterApi("getcursory", "getcursory() api",
                              new Brace::BraceApiFactory<GetCursorYExp>());
    pBraceScript->RegisterApi("getscreenwidth", "getscreenwidth() api",
                              new Brace::BraceApiFactory<GetScreenWidthExp>());
    pBraceScript->RegisterApi("getscreenheight", "getscreenheight() api",
                              new Brace::BraceApiFactory<GetScreenHeightExp>());
    pBraceScript->RegisterApi("readbuttonparam", "readbuttonparam(index) api",
                              new Brace::BraceApiFactory<ReadButtonParamExp>());
    pBraceScript->RegisterApi("readstickparam", "readstickparam(index) api",
                              new Brace::BraceApiFactory<ReadStickParamExp>());
    pBraceScript->RegisterApi("readmotionparam", "readmotionparam(index) api",
                              new Brace::BraceApiFactory<ReadMotionParamExp>());
    pBraceScript->RegisterApi("readparampackage", "readparampackage(str) api",
                              new Brace::BraceApiFactory<ReadParamPackageExp>());
    pBraceScript->RegisterApi("hasparam", "hasparam(key) api",
                              new Brace::BraceApiFactory<HasParamExp>());
    pBraceScript->RegisterApi("getintparam", "getintparam(key,def) api",
                              new Brace::BraceApiFactory<GetIntParamExp>());
    pBraceScript->RegisterApi("getfloatparam", "getfloatparam(key,def) api",
                              new Brace::BraceApiFactory<GetFloatParamExp>());
    pBraceScript->RegisterApi("getstrparam", "getstrparam(key,def) api",
                              new Brace::BraceApiFactory<GetStrParamExp>());
    pBraceScript->RegisterApi("keypress", "keypress(modifier,key) api",
                              new Brace::BraceApiFactory<KeyPressExp>());
    pBraceScript->RegisterApi("keyrelease", "keyrelease(modifier,key) api",
                              new Brace::BraceApiFactory<KeyReleaseExp>());
    pBraceScript->RegisterApi("mousepress", "mousepress(x,y,button) api",
                              new Brace::BraceApiFactory<MousePressExp>());
    pBraceScript->RegisterApi("mouserelease", "mouserelease(button) api",
                              new Brace::BraceApiFactory<MouseReleaseExp>());
    pBraceScript->RegisterApi("mousemove", "mousemove(x,y) api",
                              new Brace::BraceApiFactory<MouseMoveExp>());
    pBraceScript->RegisterApi("mousewheelchange", "mousewheelchange(x,y) api",
                              new Brace::BraceApiFactory<MouseWheelChangeExp>());
    pBraceScript->RegisterApi("touchpress", "touchpress(x,y,id) api",
                              new Brace::BraceApiFactory<TouchPressExp>());
    pBraceScript->RegisterApi("touchupdatebegin", "touchupdatebegin() api",
                              new Brace::BraceApiFactory<TouchUpdateBeginExp>());
    pBraceScript->RegisterApi("touchmove", "touchmove(x,y,id) api",
                              new Brace::BraceApiFactory<TouchMoveExp>());
    pBraceScript->RegisterApi("touchupdateend", "touchupdateend() api",
                              new Brace::BraceApiFactory<TouchUpdateEndExp>());
    pBraceScript->RegisterApi("touchend", "touchend() api",
                              new Brace::BraceApiFactory<TouchEndExp>());

    pBraceScript->RegisterApi("getbuttonstate", "getbuttonstate(id) api",
                              new Brace::BraceApiFactory<GetButtonStateExp>());
    pBraceScript->RegisterApi("setbuttonstate",
                              "setbuttonstate(uint_player_index,int_button_id,bool_value) api",
                              new Brace::BraceApiFactory<SetButtonStateExp>());
    pBraceScript->RegisterApi("setstickpos",
                              "setstickpos(uint_player_index,int_axis_id,float_x,float_y) api",
                              new Brace::BraceApiFactory<SetStickPositionExp>());
    pBraceScript->RegisterApi(
        "setmotionstate",
        "setmotionstate(uint_player_index,uint64_delta_time,float_gyro_x,float_gyro_y,float_gyro_z,"
        "float_accel_x,float_accel_y,float_accel_z) api",
        new Brace::BraceApiFactory<SetMotionStateExp>());

    pBraceScript->RegisterApi("getresultinfo", "getresultinfo() api",
                              new Brace::BraceApiFactory<GetResultInfoExp>());
    pBraceScript->RegisterApi("getlastinfo", "getlastinfo() api",
                              new Brace::BraceApiFactory<GetLastInfoExp>());
    pBraceScript->RegisterApi("gethistoryinfocount", "gethistoryinfocount() api",
                              new Brace::BraceApiFactory<GetHistoryInfoCountExp>());
    pBraceScript->RegisterApi("gethistoryinfo", "gethistoryinfo(index) api",
                              new Brace::BraceApiFactory<GetHistoryInfoExp>());
    pBraceScript->RegisterApi("getrollbackinfocount", "getrollbackinfocount() api",
                              new Brace::BraceApiFactory<GetRollbackInfoCountExp>());
    pBraceScript->RegisterApi("getrollbackinfo", "getrollbackinfo(index) api",
                              new Brace::BraceApiFactory<GetRollbackInfoExp>());
    pBraceScript->RegisterApi("setresultinfo",
                              "setresultinfo(hashtable<:int64,MemoryModifyInfo:>) api",
                              new Brace::BraceApiFactory<SetResultInfoExp>());

    pBraceScript->RegisterApi("newmemorymodifyinfo", "newmemorymodifyinfo() api",
                              new Brace::BraceApiFactory<NewMemoryModifyInfoExp>());
    pBraceScript->RegisterApi("addtoresult", "addtoresult(addr[,val_size,pid]) api",
                              new Brace::BraceApiFactory<AddToResultExp>());
    ...
    pBraceScript->RegisterApi("getpid", "getpid() api",
                              new Brace::BraceApiFactory<GetProcessIdExp>());
    pBraceScript->RegisterApi("getmodulecount", "getmodulecount() api",
                              new Brace::BraceApiFactory<GetModuleCountExp>());
    pBraceScript->RegisterApi("getmodulebase", "getmodulebase(index) api",
                              new Brace::BraceApiFactory<GetModuleBaseExp>());
    pBraceScript->RegisterApi("getmoduleaddr", "getmoduleaddr(index) api",
                              new Brace::BraceApiFactory<GetModuleAddrExp>());
    pBraceScript->RegisterApi("getmodulesize", "getmodulesize(index) api",
                              new Brace::BraceApiFactory<GetModuleSizeExp>());
    pBraceScript->RegisterApi("getmoduleid", "getmoduleid(index) api",
                              new Brace::BraceApiFactory<GetModuleIdExp>());
    pBraceScript->RegisterApi("getmodulename", "getmodulename(index) api",
                              new Brace::BraceApiFactory<GetModuleNameExp>());
    pBraceScript->RegisterApi("getmoduleprogid", "getmoduleprogid(index) api",
                              new Brace::BraceApiFactory<GetModuleProgramIdExp>());
    pBraceScript->RegisterApi("getmodulepid", "getmodulepid(index) api",
                              new Brace::BraceApiFactory<GetModuleProcessIdExp>());
    pBraceScript->RegisterApi("getheapbase", "getheapbase() api",
                              new Brace::BraceApiFactory<GetHeapBaseExp>());
    pBraceScript->RegisterApi("getheapsize", "getheapsize() api",
                              new Brace::BraceApiFactory<GetHeapSizeExp>());
    pBraceScript->RegisterApi("getstackbase", "getstackbase() api",
                              new Brace::BraceApiFactory<GetStackBaseExp>());
    pBraceScript->RegisterApi("getstacksize", "getstacksize() api",
                              new Brace::BraceApiFactory<GetStackSizeExp>());

    pBraceScript->RegisterApi("markmemdebug", "markmemdebug(addr,size[,debug,pid])",
                              new Brace::BraceApiFactory<CmdMarkMemDebugExp>());
    pBraceScript->RegisterApi("addsniffing", "addsniffing(addr,size[,step,val,pid])",
                              new Brace::BraceApiFactory<CmdAddSniffingExp>());
    pBraceScript->RegisterApi("addsniffingfromsearch", "addsniffingfromsearch(find_vals)",
                              new Brace::BraceApiFactory<CmdAddSniffingFromSearchExp>());
    pBraceScript->RegisterApi("showmem", "showmem(addr,size[,step,pid])",
                              new Brace::BraceApiFactory<CmdShowMemExp>());
    pBraceScript->RegisterApi("findmem", "findmem(find_vals), results show on ui",
                              new Brace::BraceApiFactory<CmdFindMemExp>());
    pBraceScript->RegisterApi("searchmem", "searchmem(find_vals), results show on ui",
                              new Brace::BraceApiFactory<CmdSearchMemExp>());

    pBraceScript->RegisterApi("findmemory",
                              "findmemory(start,size,step,range,find_vals[,val_size,pid])",
                              new Brace::BraceApiFactory<FindMemoryExp>());
    pBraceScript->RegisterApi("searchmemory",
                              "searchmemory(start,size,step,range,find_vals[,val_size,max_count,pid])",
                              new Brace::BraceApiFactory<SearchMemoryExp>());
    pBraceScript->RegisterApi("readmemory", "readmemory(addr[,val_size,pid])",
                              new Brace::BraceApiFactory<ReadMemoryExp>());
    pBraceScript->RegisterApi("writememory", "writememory(addr,val[,val_size,pid])",
    ...
    pBraceScript->RegisterApi("replacesourceshader",
                              ...
                              "shader_type:0--vertex 3--geometry 4--fragment 5--compute",
                              new Brace::BraceApiFactory<ReplaceSourceShaderExp>());
    pBraceScript->RegisterApi("replacespirvshader",
                              "replacespirvshader(hash,shader_type,shader_spriv_file), "
                              "shader_type:0--vertex 3--geometry 4--fragment 5--compute",
                              new Brace::BraceApiFactory<ReplaceSpirvShaderExp>());

    if (!isCallback) {
        // dmnt
        pBraceScript->RegisterApi("dmnt_file",
                                  "dmnt_file(name,module[,file_dir[,build_id]]){...}; statement",
                                  new Brace::BraceApiFactory<DmntFileExp>());
        pBraceScript->RegisterApi(
            "dmnt_if",
            "dmnt_if(exp){...}; or dmnt_if(exp){...}elseif/elif(exp){...}else{...}; or "
            "dmnt_if(exp)func(...); statement",
            new Brace::BraceApiFactory<DmntIfExp>());
        pBraceScript->RegisterApi("dmnt_loop", "dmnt_loop(reg,ct){...}; statement",
                                  new Brace::BraceApiFactory<DmntLoopExp>());

        pBraceScript->RegisterApi("dmnt_key",
                                  "dmnt_key(key) "
                                  "key:A|B|X|Y|LS|RS|L|R|ZL|ZR|Plus|Minus|Left|Up|Right|Down|LSL|"
                                  "LSU|LSR|LSD|RSL|RSU|RSR|RSD|SL|SR",
                                  new Brace::BraceApiFactory<DmntKeyExp>());
        pBraceScript->RegisterApi("dmnt_region",
                                  "dmnt_region(mem_region) mem_region:main|heap|alias|aslr",
                                  new Brace::BraceApiFactory<DmntRegionExp>());
        pBraceScript->RegisterApi("dmnt_offset",
                                  "dmnt_offset(name) "
                                  "name:no_offset|offset_reg|offset_fixed|region_and_base|region_"
                                  "and_relative|region_and_relative_and_offset",
                                  new Brace::BraceApiFactory<DmntOffsetExp>());
        pBraceScript->RegisterApi("dmnt_operand",
                                  "dmnt_operand(name) "
                                  "name:mem_and_relative|mem_and_offset|reg_and_relative|reg_and_"
                                  "offset|static_value|register_value|reg_other|restore_register|"
                                  "save_register|clear_saved_value|clear_register",
                                  new Brace::BraceApiFactory<DmntOperandExp>());

        pBraceScript->RegisterApi("dmnt_calc_offset",
                                  "dmnt_calc_offset(offset,addr,region), all type is integer",
                                  new Brace::BraceApiFactory<DmntCalcOffsetExp>());
        pBraceScript->RegisterApi("dmnt_read_mem",
                                  "dmnt_read_mem(val,addr[,val_size]), all type is integer",
                                  new Brace::BraceApiFactory<DmntReadMemExp>());
        pBraceScript->RegisterApi("dmnt_comment", "dmnt_comment(str)",
                                  new Brace::BraceApiFactory<DmntCommentExp>());
        pBraceScript->RegisterApi(
            "dmnt_store_v2a",
            "dmnt_store_v2a(mem_width,mem_region,reg,offset,val), all type is integer",
            new Brace::BraceApiFactory<DmntStoreValueToAddrExp>());
        pBraceScript->RegisterApi(
            "dmnt_gt",
            "dmnt_xxx(mem_width,mem_region,offset,val), all type is integer, xxx:gt|ge|lt|le|eq|ne",
            new Brace::BraceApiFactoryWithArgs<DmntConditionExp, const std::string&>(">"));
        pBraceScript->RegisterApi(
            "dmnt_ge",
            "dmnt_xxx(mem_width,mem_region,offset,val), all type is integer, xxx:gt|ge|lt|le|eq|ne",
            new Brace::BraceApiFactoryWithArgs<DmntConditionExp, const std::string&>(">="));
        ...
        pBraceScript->RegisterApi(
            ...
            "dmnt_xxx(mem_width,mem_region,offset,val), all type is integer, xxx:gt|ge|lt|le|eq|ne",
            new Brace::BraceApiFactoryWithArgs<DmntConditionExp, const std::string&>("!="));
        pBraceScript->RegisterApi("dmnt_load_v2r", "dmnt_load_v2r(reg,val), all type is integer",
                                  new Brace::BraceApiFactory<DmntLoadRegWithValueExp>());
        pBraceScript->RegisterApi(
            "dmnt_load_m2r",
            "dmnt_load_m2r(mem_width[,mem_region],reg,offset), all type is integer",
            new Brace::BraceApiFactory<DmntLoadRegWithMemoryExp>());
        pBraceScript->RegisterApi(
            "dmnt_store_v2m",
            "dmnt_store_v2m(mem_width,mem_reg,reg_inc_1or0,val[,offset_reg]), all type is integer",
            new Brace::BraceApiFactory<DmntStoreValueToMemoryExp>());
        pBraceScript->RegisterApi(
            "dmnt_legacy_add",
            "dmnt_legacy_xxx(mem_width,reg,val), all type is integer, "
            "xxx:add|sub|mul|lshift|rshift",
            new Brace::BraceApiFactoryWithArgs<DmntLegacyArithExp, const std::string&>("+"));
        pBraceScript->RegisterApi(
            "dmnt_legacy_sub",
            "dmnt_legacy_xxx(mem_width,reg,val), all type is integer, "
            "xxx:add|sub|mul|lshift|rshift",
        ...
        pBraceScript->RegisterApi(
            "dmnt_keypress",
            "dmnt_keypress(key1,key2,...); all type is integer, key can get by dmnt_key(const)",
            new Brace::BraceApiFactory<DmntKeyPressExp>());
        pBraceScript->RegisterApi(
            "dmnt_add",
            "dmnt_xxx(mem_width,result_reg,lhs_reg,rhs[,rhs_is_val_1or0]), all type is integer, "
            "xxx:add|sub|mul|lshift|rshift|and|or|not|xor|mov",
            new Brace::BraceApiFactoryWithArgs<DmntArithExp, const std::string&>("+"));
        pBraceScript->RegisterApi(
            "dmnt_sub",
            "dmnt_xxx(mem_width,result_reg,lhs_reg,rhs[,rhs_is_val_1or0]), all type is integer, "
            "xxx:add|sub|mul|lshift|rshift|and|or|not|xor|mov",
        ...
        pBraceScript->RegisterApi(
            ...
            "xxx:add|sub|mul|lshift|rshift|and|or|not|xor|mov",
            new Brace::BraceApiFactoryWithArgs<DmntArithExp, const std::string&>("="));
        pBraceScript->RegisterApi("dmnt_store_r2m",
                                  "dmnt_store_r2m(mem_width,src_reg,mem_reg,reg_inc_1or0,[offset_"
                                  "type,offset_or_reg_or_region[,offset]]), all type is integer",
                                  new Brace::BraceApiFactory<DmntStoreRegToMemoryExp>());
        pBraceScript->RegisterApi(
            "dmnt_reg_gt",
            "dmnt_reg_xxx(mem_width,src_reg,opd_type,val1[,val2]), all type is integer, "
            "xxx:gt|ge|lt|le|eq|ne",
            new Brace::BraceApiFactoryWithArgs<DmntRegCondExp, const std::string&>(">"));
        pBraceScript->RegisterApi(
            "dmnt_reg_ge",
            "dmnt_reg_xxx(mem_width,src_reg,opd_type,val1[,val2]), all type is integer, "
            "xxx:gt|ge|lt|le|eq|ne",
        ...
        pBraceScript->RegisterApi(
            ...
            "xxx:gt|ge|lt|le|eq|ne",
            new Brace::BraceApiFactoryWithArgs<DmntRegCondExp, const std::string&>("!="));
        pBraceScript->RegisterApi("dmnt_reg_sr",
                                  "dmnt_reg_sr(dest_reg,src_reg,opd_type), all type is integer",
                                  new Brace::BraceApiFactory<DmntRegSaveRestoreExp>());
        pBraceScript->RegisterApi("dmnt_reg_sr_mask",
                                  "dmnt_reg_sr_mask(opd_type,mask), all type is integer",
                                  new Brace::BraceApiFactory<DmntRegSaveRestoreWithMaskExp>());
        pBraceScript->RegisterApi(
            "dmnt_reg_rw",
            "dmnt_reg_rw(static_reg_index,reg), all type is integer, static_reg_index: 0x00 to "
            "0x7F for reading or 0x80 to 0xFF for writing",
            new Brace::BraceApiFactory<DmntRegReadWriteExp>());
        pBraceScript->RegisterApi("dmnt_pause", "dmnt_pause()",
                                  new Brace::BraceApiFactory<DmntPauseExp>());
        pBraceScript->RegisterApi("dmnt_resume", "dmnt_resume()",
                                  new Brace::BraceApiFactory<DmntResumeExp>());
        pBraceScript->RegisterApi(
            "dmnt_debug", "dmnt_debug(mem_width,log_id,opd_type,val1[,val2]), all type is integer",
            new Brace::BraceApiFactory<DmntDebugLogExp>());
    }
}
                ...
                if (pComp->GetSyntaxType() == DslParser::ISyntaxComponent::TYPE_FUNCTION) {
                    ...
                    if (!pFunc->IsHighOrder()) {
                        ...
                        for (int ix = 0; ix < pFunc->GetParamNum(); ++ix) {
                            ...
                            if (pParam->GetSyntaxType() !=
                                DslParser::ISyntaxComponent::TYPE_VALUE) {
                                maybeCommand = false;
                                break;
                            }
                        }
