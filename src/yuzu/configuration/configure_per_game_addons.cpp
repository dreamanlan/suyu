...
void ConfigurePerGameAddons::ReloadList() {
    ...
    Common::FS::RemoveFile(Common::FS::GetYuzuPath(Common::FS::YuzuPath::CacheDir) / "game_list" /
...
void ConfigurePerGameAddons::OnPatchCreateClick(bool checked) {
    std::filesystem::path addon_path =
        Common::FS::GetYuzuPath(Common::FS::YuzuPath::LoadDir) / fmt::format("{:016X}", title_id);
