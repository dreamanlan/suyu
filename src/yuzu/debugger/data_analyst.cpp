...
namespace Core {
    ...
    void MainThreadCaller::RequestLogToView(std::string&& msg) {
        if (impl) {
            impl->RequestAsyncOperation([this, moved_msg = std::move(msg)]() {
                impl->data_widget.AddLog(moved_msg);
            });
        }
    }
...
std::filesystem::path DataAnalystWidget::GetModLoadPath()const {
    ...
    std::filesystem::path path = Common::FS::GetYuzuPath(Common::FS::YuzuPath::LoadDir) / fmt::format("{:016X}", program_id);
